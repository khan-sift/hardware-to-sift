...
| Protobuf streaming | gRPC | Confirmed, in use | Stream arbitrary protocol buffers. Raw gRPC only, no high-level client wrapper. Schema registered out of band via `ProtobufDescriptorService.AddProtobufDescriptor` (a serialized `FileDescriptorSet`); fields auto-map to channels, with units and other behavior set by `sift.protobuf_descriptors.v2` field options. See lab 02. |
...
1. Empirical end-to-end run: closed. On 2026-06-09 the synthetic streamer (`smoke_synthetic.py`) streamed live into the dev environment, and the `sine.value` channel on the `synthetic-smoke` asset renders the expected clean sine in the Sift UI. This confirms credentials, the dev-env URL, and the full ingestion pipeline through the `sift_py` config-based path. The dotted channel name appeared as the `sine` group with a `value` leaf, confirming the naming convention the labs rely on. The `sift_client` async path is now also live-confirmed (2026-06-09, the `synthetic-smoke-sift-client` asset renders the same sine via the native `sift_stream` bindings), so both ingestion clients are validated end to end. The raw-gRPC protobuf path (lab 02) is now live-confirmed too (2026-06-09, the `protobuf-smoke` asset, four `DroneState.*` channels populated via `AddProtobufDescriptor` then `IngestArbitraryProtobufDataStream`). Sift namespaces the channels under the message type (`DroneState.roll`, and so on), and units come back as a placeholder unless fields are annotated with the `(units)` option. With that, both gRPC streaming mechanisms are validated live end to end. The data-import path (lab 03) is also live-confirmed (2026-06-09): both CSV and Parquet imported into the `drone-import-demo` asset as separate runs, both jobs `SUCCEEDED`, with the explicit `units` (rad, V) propagating through to the UI. Of the framework's five mechanisms, the two streaming paths and data import are now validated live; schemaless JSON over REST and Influx line protocol (lab 05) remain to be built. The CAN/DBC lab (lab 04), which decodes frames and streams them through the config path, is also live-confirmed (2026-06-09, the `can-bus-demo` asset, four `MESSAGE.signal` channels with units pulled straight from the DBC: kph, degC, V, A). So all four built labs are validated live; only lab 05's two mechanisms are unbuilt.
